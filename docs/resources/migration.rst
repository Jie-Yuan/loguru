...
Loguru moves the standard |Logger| configuration to the :ref:`sink <sink>` definition. The |logger.setLevel| and |logger.addFilter| are suppressed by the configured sink parameters. The |propagate| attribute and |disable| method can be replaced by the ``filter`` option. The |makeRecord| method can be replaced using the ``record["extra"]`` dict.
...
Standard ``logging`` requires you to create an |Handler| object and then call |addHandler|. Using Loguru, the started handlers are added using |add| and then shared across all modules using ``logger``. The added sink defines how the handler should manage incoming logging messages, as would do |handle| or |emit|.
...
The formatted exception will be fully descriptive, to prevent that, make sure to use ``backtrace=False`` while adding your sink.
...
Replacing ``dictConfig()`` function
-----------------------------------

The |dictConfig| function is replaced by the |configure| method.

This does not accept ``config.ini`` files, though, so you have to handle that yourself using your favorite format.


Replacing other less popular functions
--------------------------------------
